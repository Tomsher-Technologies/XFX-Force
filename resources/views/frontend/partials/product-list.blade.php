<section aria-labelledby="products-heading" class="pb-24 pt-6" id="product-list-wrapper">
                    ...
                    <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-3">
                        <div class="lg:col-span-3">
                            <div class="grid grid-cols-3 gap-3">
                                ...
                                @endphp
                                <div>
                                    <a href="{{ route('product.details', $product->id) }}" class="product-card w-full relative border-hidden rounded-[20px] overflow-hidden bg-[#1E2225] flex flex-col items-start justify-start transition-all duration-600">
                    ...
                    <div class="flex flex-col gap-[15px]">
                        ...
                        @php
                            ...
                            $firstStock = $product->stocks->first();
                        @endphp
                        <div class="product-card-list grid grid-cols-4 overflow-hidden bg-black/30 backdrop-blur-[60px] rounded-[20px]" onclick="window.location= '{{ route('product.details', $product['id']) }}'" style="cursor: pointer;">
                            <div class="product-image col-span-1">
                                <img src="{{ Storage::url($product->thumbnail_img) }}" alt="" title="" class="h-full w-full object-cover object-center">
                            </div>
                            <div class="col-span-2 p-[30px]">
                                <h4 class="text-white text-[20px] mb-[20px] leading-[30px] font-medium line-clamp-2">{{$product->name}}</h4>
                                @php
                                    ...
                                    <ul>
                                        ...
                                        <li class="flex flex-row py-[15px] border-b-1 border-[#282B34] justify-between">
                                            <div class="title flex flex-row gap-[20px] w-full">
                                                ...
                                                <h5 class="text-[15px] text-[#636671] uppercase">{{ $specification->specification->main_title }}</h5>
                                            </div>
                                            <div class="value w-full">
                                                <h6 class="text-[15px] text-[#C4C4C4] text-left">@if($specification->specificationItem)
                            ...
                            <div class="col-span-1 p-[30px] flex flex-col justify-between">
                                <div class="price w-full flex flex-col items-end">
                                    <h5 class="price flex flex-row text-[#2A7CFF] text-right text-[30px] m-[0] font-bold align-center items-center gap-[10px]">
                                        <img src="{{ asset('assets/images/aed.svg') }}" class="w-[25px] h-[25px]" alt="AED" title="Symbol of AED">{{ $firstStock->offer_price ?? $firstStock->price }} 
                                    </h5>
                                    @if($firstStock->offer_price)
