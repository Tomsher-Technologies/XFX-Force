    ...
    <div class="flex flex-col divide-y-1 divide-[#282B34] mb-[20px]">
        @foreach($pcBuilderItems as $builderId => $items)
            <div class="flex flex-row justify-between mb-2 align-center items-center">
                ...
                class="flexcursor-pointer items-center justify-center gap-3 px-6 py-4 rounded-xl border border-[#2E363E] text-gray-400 font-medium uppercase text-[13px] hover:bg-[#252C33] hover:text-white transition-all duration-300">
                ...
                <div class="product-cart-item flex flex-col md:col-col md:grid md:grid-cols-12 gap-[20px] xl:gap-[50px] w-full bg-black/30 backdrop-blur-[60px] p-[20px] md:p-[30px] rounded-[20px]">
                    <div class="col-span-7 flex flex-col md:flex-row gap-[20px] md:gap-[30px]">
                        <div class="product-img h-[150px] md:h-auto w-full md:w-[200px] relative z-[1] bg-[#1E2225] rounded-[10px] overflow-hidden items-center justify-center flex">
                            ...
                            <img src="{{ $image }}" class="absolute object-fit object-center w-auto md:w-full h-[150px] md:h-auto cursor-pointer" alt="{{ $item->product_stock->stock_title ?? $item->product->name ?? '' }}" title="{{ $item->product_stock->stock_title ?? $item->product->name ?? '' }}" onclick="window.location='{{ route('product.details' ,[$item->product->slug, $item->product_stock->sku]) }}'">
