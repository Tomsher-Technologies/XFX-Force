...
<section class="m-[16px] xl:m-0 bg-[#0F161B] h-screen p-[0px] xl:p-[30px] flex flex-col overflow-x-hidden">
    <main class="grid grid-cols-1 xl:grid-cols-12 h-full xl:h-[calc(100vh-60px)] overflow-y-auto rounded-[20px] custom-scrollbar">
            ...
            <div class="p-[30px] border-t-1 border-[#2E363E] flex flex-col gap-[20px]">
                <div>
                    ...
                    <div class="price w-full flex flex-row items-end gap-[15px]">
                        <h5 class="price flex flex-row text-white text-left text-[20px] m-[0] font-bold align-center items-center gap-[10px] leading-[35px]">
        ...
        <section class="col-span-7 h-[calc(100vh-32px)] xl:h-[calc(100vh-60px)] overflow-y-auto no-scrollbar overflow-x-hidden px-0 xl:px-[30px] pb-[100px] xl:pb-0" id="products-list-page">
            ...
            <div class="sticky top-0 bg-[#1C2228] border border-[#1E2529] p-[15px] md:p-[30px] rounded-[20px] flex flex-col justify-between group transition-all gap-[30px] z-[1]">
                ...
                <div class="flex flex-col md:flex-row justify-between w-full gap-[15px] md:gap-[0px]">
                    <div class="flex flex-col gap-[5px] w-full">
                        <h2 class="text-white text-[20px] font-bold category-heading"></h2>
                ...
                <div class="hidden xl:flex items-center bg-[#252B31] border border-[#1E2529] rounded-[15px] h-[50px] relative">
                    ...
                    <div class="relative h-full border-r border-[#2E3239] min-w-[180px]">
                        <button onclick="toggleDropdown('model-menu')" class="flex items-center px-6 w-full h-full hover:bg-[#252C33] transition-colors">
                    ...
                    <div class="flex items-center h-full px-6 gap-8">
                        <button onclick="toggleDropdown('mega-menu')" class="flex items-center gap-3 text-white font-medium text-[14px] hover:text-[#2A7CFF] transition-colors">
            ...
            <div id="products-loader" class="hidden text-center p-4 text-white text-[15px] font-bold">
                Loading products...
            </div>
        ...
        <aside id="details-sidebar" class=" fixed inset-x-4 inset-y-10 z-[150] translate-y-full opacity-0 pointer-events-none transition-all duration-300 xl:translate-y-0 xl:opacity-100 xl:pointer-events-auto xl:relative xl:inset-0 xl:z-auto xl:col-span-3 bg-[#1C2228] border border-[#1E2529] rounded-[20px] flex flex-col shadow-2xl h-[calc(100vh-80px)] xl:h-[calc(100vh-60px)] overflow-hidden">
            ...
            <div id="default-view" class="flex flex-col gap-[15px] justify-center items-center h-full p-6">
                ...
                <h2 class="text-[18px] text-gray-400 font-bold text-center">Select a component</h2>
        ...
        <aside id="summary-sidebar" class="hidden relative xl:fixed mb-[100px] pointer-events-none transition-all duration-300 xl:translate-y-0 xl:opacity-100 xl:pointer-events-auto xl:relative xl:inset-0 xl:z-auto xl:col-span-3 bg-[#1C2228] border border-[#1E2529] rounded-[20px] flex flex-col shadow-2xl h-fit overflow-hidden">
            <div class="flex flex-row justify-between items-center p-[20px] md:p-[30px] border-b border-[#2E363E] flex-shrink-0">
                <div>
                    <h2 class="text-white text-[20px] leading-[30px] font-bold">Summary</h2>
            ...
            <div class="flex flex-col gap-[20px] justify-between items-center p-[20px] md:p-[30px] border-t border-[#2E363E] bg-[#1C2228] rounded-b-[20px]">
                <div class="w-full">
                    ...
                    <div class="price w-full flex flex-row items-end gap-[15px]">
                        <h5 class="flex flex-row text-white text-left text-[20px] m-[0] font-bold items-center gap-[10px] leading-[35px]">
                            <svg width="15" height="13" viewBox="0 0 15 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span id="total_price_with_tax" class="text-white text-left text-[20px] font-bold leading-[35px]">0.00</span>
                        </h5>
                    </div>
                </div>
                <div class="w-full">
        ...
        <div class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[92%] md:block xl:hidden">
            <div class="bg-[#1E2225]/90 backdrop-blur-lg border border-white/10 rounded-[22px] shadow-2xl grid grid-cols-4 h-[70px]">
                ...
                <button onclick="toggleMobileWidget('mobile-filter-panel', event)" class="flex flex-col items-center justify-center gap-1 text-gray-400 border-r border-white/5">
                    ...
                    <span class="text-[9px] font-bold uppercase">Filter</span>
                </button>
                ...
                <button onclick="window.location.reload()" class="flex flex-col items-center justify-center gap-1 text-gray-400">
                    ...
                    <span class="text-[9px] font-bold uppercase">Reset</span>
                </button>
            </div>
        </div>
        <!--//navigation menu-->
...
<script>
    ...
    function selectProduct(buttonElement, qty = 1) {
        // Scope to the nearest container to avoid interfering with other products
        ...
        const index = buildData[categoryId].findIndex(v => v.variant_id == variantId);
        if (index > -1) {
            // Deselect
            buildData[categoryId].splice(index, 1);
            productCard.classList.remove('product-selected');
        } else {
            // Add to buildData
    ...
    function isCategoryLimitExceeded(categoryElement, maxSelect, categoryId) {

        if (!categoryElement) return false;

        // null, empty or undefined → unlimited
        if (!maxSelect || maxSelect === "null") return false;

        const maxLimit = parseInt(maxSelect);

        const selectedProducts = document.querySelectorAll(
            `.product-card.product-selected[data-category-id="${categoryId}"]`
        );

        let totalSelectedProducts = selectedProducts.length;

        // If buildData exists and category exists
        if (buildData && buildData[categoryId]) {
            totalSelectedProducts = Math.max(
                totalSelectedProducts,
                buildData[categoryId].length
            );
        }

        console.log("selected products length = " + totalSelectedProducts);
        console.log("category maximum limit = " + maxLimit);

        return totalSelectedProducts > maxLimit;
    }
    ...
    function isMinSelectionSatisfied(categoryId, minSelect) {
        if (!minSelect) return true;
        const min = parseInt(minSelect);
        const selectedProducts = document.querySelectorAll(`.product-card.product-selected[data-category-id="${categoryId}"]`);
        return selectedProducts.length >= min;
    }
